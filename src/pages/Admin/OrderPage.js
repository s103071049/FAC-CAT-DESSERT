...
const OrderPage = () => {
  ...
  useEffect(() => {
    ...
    if (selectOrderStatus === 2) return setCurrentOrders(fakeOrders.filter(order => order.is_accepted !== 0))
    if (selectOrderStatus === 3) return setCurrentOrders(fakeOrders)
    if (selectOrderStatus === 4) return setCurrentOrders(fakeOrders.filter(order => order.is_accepted === 3))
    if (selectOrderStatus === 5) return setCurrentOrders(fakeOrders.filter(order => order.is_accepted === 1 || order.is_accepted === 2))
  }, [selectOrderStatus])
